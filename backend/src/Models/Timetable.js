  ...
  periods: {
    ...
    validate: {
      validator: function(periods) {
        ...
        if (this.isLab) {
          const isFirstThree = periods.every(period => [1, 2, 3].includes(period));
          const isLastThree = periods.every(period => [4, 5, 6].includes(period));
...
timetableSchema.pre('save', function(next) {
  ...
  for (let slot of this.timeSlots) {
    ...
    if (uniquePeriods.length !== slot.periods.length) {
      return next(new Error('Duplicate periods are not allowed in the same time slot'));
    }
...
timetableSchema.statics.findBySemesterAndYear = function(semester, academicYear) {
