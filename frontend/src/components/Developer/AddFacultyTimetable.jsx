...
const AddFacultyTimetable = () => {
  ...
  const addTimeSlot = () => {
    if (!currentSlot.subject || !currentSlot.branch || !currentSlot.room) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem('adminToken');
      const response = await axios.post(`${API_BASE_URL}/admin/timetable/faculty`, formData, {
        headers: { Authorization: `Bearer ${token}` }
      });

      if (response.data.success) {
        alert('Faculty timetable created successfully!');
        navigate('/developer');
      }
    } catch (error) {
  ...
  return (
    <div className="add-timetable-container">
      ...
      <div className="form-container">
        <h2>Create Faculty Timetable</h2>
        ...
        <form onSubmit={handleSubmit} className="timetable-form">
          ...
          <div className="time-slot-section">
            ...
            {formData.timeSlots.length > 0 && (
              <div className="added-slots">
                <h4>Added Time Slots ({formData.timeSlots.length})</h4>
                  ...
                  {formData.timeSlots.map((slot, index) => (
                    <div key={index} className="slot-item">
                      <div className="slot-details">
                        ...
                        <br />
                        {slot.subject} | {slot.branch} | {slot.semester} | {slot.room}
          ...
          <div className="form-actions">
            <button type="submit" className="btn-primary" disabled={loading}>
              {loading ? 'Creating Timetable...' : 'Create Timetable'}
            </button>
...
export default AddFacultyTimetable;
