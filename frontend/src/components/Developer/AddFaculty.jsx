...
const AddFaculty = () => {
  ...
  const handleFormSubmit = async (e) => {
    ...
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/admin/faculty/create`,
        formData,
        {
  ...
  const handleCSVSubmit = async (e) => {
    ...
    try {
      const token = localStorage.getItem("adminToken");
      const response = await axios.post(
        `${API_BASE_URL}/admin/faculty/bulk-create`,
        {
          facultyData: csvData,
        },
        {
      ...
      if (response.data.success) {
        alert(
          `Successfully created ${response.data.data.created.length} faculty accounts!`,
        );
        setCsvData([]);
        setCsvFile(null);
      }
      ...
      {uploadMode === "form" ? (
        ...
        <div className="csv-container">
          ...
          <div className="csv-info">
            ...
            <ul>
              <li>
                Headers: name, email, password, employeeId, department,
                designation, phoneNumber, branch, subjects
              </li>
              <li>
                Branch and subjects should be semicolon-separated (e.g.,
                "CSE;IT" or "SE;DBMS;Web Tech")
              </li>
          ...
          <form onSubmit={handleCSVSubmit} className="csv-form">
            ...
            {csvData.length > 0 && (
              <div className="csv-preview">
                ...
                <div className="preview-table">
                  ...
                  {csvData.length > 5 && (
                    <p>... and {csvData.length - 5} more records</p>
                  )}
                </div>
              </div>
            )}
            ...
            <div className="form-actions">
              ...
              >
                {loading
                  ? "Uploading..."
                  : `Upload ${csvData.length} Faculty Records`}
              </button>
              <button
                type="button"
                onClick={() => navigate("/developer")}
